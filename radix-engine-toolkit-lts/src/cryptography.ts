...
export abstract class PublicKey {
  ...
  hexString = (): string => Toolkit.Convert.Uint8Array.toHexString(this.bytes);
...
export abstract class PrivateKey implements Signer {
  ...
  static Secp256k1 = class extends PrivateKey implements Signer {
    ...
    publicKeyBytes(): Uint8Array {
      return Toolkit.publicKey({
        privateKey: this.bytes,
        kind: this.curve,
      }).publicKey;
    }
    publicKeyHex(): string {
      return Toolkit.Convert.Uint8Array.toHexString(this.publicKeyBytes());
    }
...
export type SignatureSource<T> = Signer | T | Toolkit.SignatureFunction<T>;
...
export const resolveSignatureSource = <T>(
  ...
  if (typeof source === "function") {
    return (source as Toolkit.SignatureFunction<T>)(messageHash);
