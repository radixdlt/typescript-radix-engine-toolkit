...
describe("Statically Validate Transaction", () => {
  it("static validation succeeds", async () => {
    let output = await RawRadixEngineToolkit.staticallyValidateTransaction(
    ...
    expect(
      output instanceof StaticallyValidateTransactionOutputInvalid
    ).toBeTruthy();
    expect(
      (output as StaticallyValidateTransactionOutputInvalid).error
